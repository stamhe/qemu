...
static void build_pci_bus_end(PCIBus *bus, void *bus_state)
{
    ...
    if (bus->parent_dev) {
        op = 0x82; /* DeviceOp */
        build_append_nameseg(bus_table, "S%0.02X_",
    ...
    if (bsel) {
        ...
        for (i = 0; i < PCI_SLOT_MAX; i++) {
            ...
            build_append_byte(notify, 0x86); /* NotifyOp */
            build_append_nameseg(notify, "S%0.02X_", PCI_DEVFN(i, 0));
            build_append_byte(notify, 0x69); /* Arg1Op */
    ...
    if (bus_hotplug_support || child->notify_table->len || !bus->parent_dev) {
        ...
        if (bus->parent_dev) {
            ...
            build_append_nameseg(parent->notify_table, "S%0.02X_",
                                 bus->parent_dev->devfn);
...
typedef enum {
    ...
    MEM_AFFINITY_HOT_PLUGGABLE = (1 << 1),
